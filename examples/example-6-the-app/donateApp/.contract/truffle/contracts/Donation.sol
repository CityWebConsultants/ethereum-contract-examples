...
//@todo create dedicated safe math library.
//@todo update deployment workflow to accomodate publishing single safe math library and using as dependency.
...
contract Donation {
  ...
  function getTotalWeight() private returns (uint) {
    ...
    for(i=0;i<payeesIndexSize;i++) {
       if (payees[payeesIndex[i]].status == true) {
         totalWeight = add(totalWeight, payees[payeesIndex[i]].weight);
       }
    }
  ...
  if (msg.value == 0) throw;
    ...
    uint totalWeight = getTotalWeight();
    uint weightedUnit = amount / totalWeight;

    for (i=0;i<payeesIndexSize;i++) {
       if (payees[payeesIndex[i]].status == true) {
        // uint weight = div(totalWeight, payees[payeesIndex[i]].weight);
         uint payeeAmount = mul(weightedUnit, payees[payeesIndex[i]].weight);
         payees[payeesIndex[i]].balance = add(payees[payeesIndex[i]].balance, payeeAmount);
       }
    }
  ...
  function updatePayeeWeight(address _payee, uint _weight) isOwner {
    PayeeAction(_payee, 'weight updated');
    payees[_payee].weight = _weight;
  }
  ...
  function withdraw(uint amount) payable isPayee {
    ...
    if (!msg.sender.send(amount)) throw;
        ...
        payees[msg.sender].balance = sub(payees[msg.sender].balance, amount);
  }
  ...
  function transferBalance(address _from, address _to, uint amount) isOwner {
    ...
    payees[_from].balance = sub(payees[_from].balance,amount);
    payees[_to].balance = add(payees[_to].balance, amount);
  ...
  // @todo change name of function to specify payee
  //@todo rename this function so it doesn not conflict with web3 native calls
  // also rename balance to userContractBalance
  function getBalance(address _address) isPayee returns (uint) {
    return payees[_address].balance;
  }

  function getUserBalance(address _address) isPayee returns (uint) {
    return payees[_address].balance;
  }
